...
trait WithOrderTest
{
        ...
        for ( $i = 0; $i < $this->count; $i++ ) {
            ...
            if ( is_callable( $callback ) ) {
                ...
                if ( $this->shouldMakePayment ) {
                    ...
                    $this->assertEquals(
                        $currency->define( Arr::get( $singleResponse[ 'order-creation' ], 'data.order.subtotal' ) )->toFloat(),
                        $currency->define( collect( Arr::get( $singleResponse[ 'order-creation' ], 'data.order.products', [] ) )
                            ->sum( 'total_price' ) )->toFloat()
                    );
    ...
    protected function attemptCouponUsage(): void
    {
        /**
         ...
         */
        $faker = Factory::create();
        $customerCoupon = CustomerCoupon::where( 'customer_id', '!=', 0 )
            ->with( [ 'coupon.products', 'coupon.categories', 'customer' ] )
            ->latest( 'id' )
            ->first();

        $this->assertInstanceOf( CustomerCoupon::class, $customerCoupon, __( 'No customer coupon was generated by the reward.' ) );

        $customer = $customerCoupon->customer;
        $products = $this->retrieveCouponEligibleProducts( $customerCoupon->coupon );
        $shippingFees = 0;
    ...
    private function retrieveCouponEligibleProducts( Coupon $coupon ): SupportCollection
    {
        $eligibleProductIds = $coupon->products->pluck( 'product_id' );
        $eligibleCategoryIds = $coupon->categories->pluck( 'category_id' );

        $products = Product::query()
            ->withStockEnabled()
            ->notGrouped()
            ->notInGroup()
            ->when( $eligibleProductIds->isNotEmpty(), fn( $query ) => $query->whereKey( $eligibleProductIds ) )
            ->when( $eligibleCategoryIds->isNotEmpty(), fn( $query ) => $query->whereIn( 'category_id', $eligibleCategoryIds ) )
            ->whereRelation( 'unit_quantities', 'quantity', '>', 100 )
            ->with( 'unit_quantities', fn( $query ) => $query->where( 'quantity', '>', 100 ) )
            ->limit( 3 )
            ->get();

        $this->assertNotEmpty( $products, __( 'No stocked product satisfies all product and category restrictions for the reward coupon.' ) );
