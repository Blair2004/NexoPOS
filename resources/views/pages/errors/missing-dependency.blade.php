    ...
    <div class="h-full w-full overflow-y-auto pb-10 bg-gradient-to-bl from-pink-400 to-blue-400 flex items-center justify-center">
        <div class="w-full md:w-1/2 xl:1/3 flex items-center flex-col justify-center">
            ...
            <div class="flex md:flex-row flex-col -mx-4 my-4 flex-wrap w-56 lg:w-auto">
                <div class="px-4 mb-4"><a class="block w-full lg:w-auto btn bg-white text-gray-700" href="{{ ! empty( request()->query( 'back' ) ) ? urldecode( request()->query( 'back' ) ) : url()->previous() }}"><i class="las la-car-crash"></i> {{ __( 'Go Back' ) }}</a></div>
                <div class="px-4 mb-4"><a class="block w-full lg:w-auto btn bg-white text-gray-700" href="{{ url()->current() . '?back=' . urlencode( request()->query( 'back' ) ?? url()->previous() )  }}"><i class="las la-sync"></i> {{ __( 'Continue' ) }}</a></div>
