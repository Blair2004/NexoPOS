<template>
    ...
    <p :key="index" v-for="(error,index) of field.errors" class="text-xs ns-error">
        ...
        <slot v-if="error.identifier === 'same'" :name="error.identifier">{{ __( 'The field must be similar to "{other}""' ).replace( '{other}', error.fields.filter( _f => _f.name === error.rule.value )[0].label ) }}</slot>
        <slot v-if="error.identifier === 'different'" :name="error.identifier">{{ __( 'The field must be different from "{other}""' ).replace( '{other}', error.fields.filter( _f => _f.name === error.rule.value )[0].label ) }}</slot>
    </p>
</template>
<script lang="ts">
