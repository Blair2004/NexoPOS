<template>
    <div class="bg-surface-tertiary shadow-lg w-95vw h-95vh md:h-3/5-screen md:w-2/4-screen flex flex-col overflow-hidden">
        ...
        <div class="flex-auto overflow-hidden flex flex-col">
            ...
            <div class="overflow-y-auto flex-auto relative">
                <ul>
                    <li v-for="product of products" :key="product.id" @click="addToCart( product )" class="hover:bg-surface-primary cursor-pointer p-2 flex justify-between border-b border-surface-secondary">
                        <div class="text-primary">
                            {{ product.name }}
                        </div>
                        <div></div>
                    </li>
                    <li v-if="products.length === 0" class="text-primary text-center p-2">{{ __( 'There is nothing to display. Have you started the search ?' ) }}</li>
                </ul>
