<template>
    <div class="flex min-h-48 flex-col gap-4">
        <div v-if="loading" class="flex min-h-48 flex-col items-center justify-center gap-2 text-fontcolor-soft">
        ...
        <article
            ...
            :key="group.on"
            class="overflow-hidden rounded-lg border border-box-edge bg-box-background text-fontcolor shadow"
        >
            <header class="flex flex-col gap-3 border-b border-box-edge p-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="min-w-0">
                    <h3 class="font-semibold">{{ events[group.on]?.label || group.on }}</h3>
                    <p class="mt-1 text-sm text-fontcolor-soft">
                        {{ __( 'Every action is posted together as one balanced journal.' ) }}
                    </p>
                </div>
                <div class="flex shrink-0 gap-2">
                    <div class="ns-button info">
                        <button type="button" class="rounded px-3 py-2" :disabled="group.saving || !isBalanced( group )" @click="saveGroup( group )">
                            <i class="las la-save" aria-hidden="true"></i>
                            {{ group.saving ? __( 'Saving&' ) : __( 'Save Group' ) }}
                        </button>
                    </div>
                    <div v-if="!group.locked" class="ns-button error">
                        <button type="button" class="rounded px-3 py-2" :disabled="group.saving" @click="confirmDeleteGroup( group )">
                            <i class="lar la-trash-alt" aria-hidden="true"></i>
                            <span class="sr-only">{{ __( 'Delete group' ) }}</span>
                        </button>
                    </div>
                </div>
            </header>
            ...
            <div class="flex flex-col gap-3">
                <div class="p-4">
                    ...
                    :key="line.local_key || line.id"
                    class="grid grid-cols-1 gap-3 rounded-lg border border-box-elevation-edge bg-box-elevation-background p-3 md:grid-cols-[2fr_1fr_2fr_auto]"
                >
                        <label class="flex min-w-0 flex-col gap-1 text-sm">
                            ...
                            >
                                ...
                                <optgroup :label="__( 'Accounts' )">
                                    <option v-for="account in accounts" :key="account.id" :value="'account:' + account.id">
                                        {{ account.account }}  -  {{ account.name }}
                                    </option>
                                </optgroup>
                                <optgroup v-if="roleEntries( group.on ).length" :label="__( 'Dynamic Accounts' )">
                                    <option v-for="[role, definition] in roleEntries( group.on )" :key="role" :value="'role:' + role">
                                        {{ definition.label }}
                                    </option>
                                </optgroup>
                            </select>
                        ...
                        <label class="flex flex-col gap-1 text-sm">
                            ...
                            <select v-model="line.effect" :disabled="group.saving" class="h-10 rounded border border-input-edge bg-input-background px-3 text-fontcolor outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:bg-input-disabled">
                        ...
                        <label class="flex min-w-0 flex-col gap-1 text-sm">
                            ...
                            <select v-model="line.amount_source" :disabled="group.saving" class="h-10 w-full rounded border border-input-edge bg-input-background px-3 text-fontcolor outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:bg-input-disabled">
                                <option v-for="[source, label] in sourceEntries( group.on )" :key="source" :value="source">{{ label }}</option>
                            </select>
                        </label>
                        ...
                        <div class="flex justify-center flex-col">
                            <div class="flex items-end justify-end gap-1">
                                <button
                                    ...
                                    :disabled="group.saving || index === 0"
                                    :aria-label="__( 'Move action up' )"
                                ...
                                <button
                                    ...
                                    :aria-label="__( 'Move action down' )"
                                    @click="moveLine( group, index, 1 )"
                                >
                                ...
                                <button
                                    ...
                                    @click="confirmRemoveLine( group, line )"
                                >
                    ...
                    <div v-if="group.error" class="rounded border border-error-primary bg-box-elevation-background p-3 text-sm text-error-tertiary" role="alert">
                        {{ group.error }}
                    </div>
                </div>

                <footer class="flex flex-col p-4 gap-3 border-t border-box-edge pt-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-sm">
                    ...
                    <div class="ns-button default">
                        <button type="button" class="rounded px-3 py-2" :disabled="group.saving" @click="addLine( group )">
...
export default {
    ...
    mounted() {
        forkJoin([
            nsHttpClient.get( '/api/transactions-accounts' ),
            nsHttpClient.get( '/api/transactions/rules' ),
        ]).subscribe({
    ...
    methods: {
        ...
        confirmDeleteGroup( group: RuleGroup ) {
            ...
            Popup.show( nsConfirmPopup, {
                ...
                message: __( 'Delete this complete rule group? New activity for this event will not post until a valid group is saved.' ),
                onAction: (confirmed: boolean) => {
                    if ( confirmed ) {
        ...
        saveGroup( group: RuleGroup ) {
            ...
            group.saving = true;
            nsHttpClient.post( '/api/transactions/rules', {
                rule: {
                    id: group.id,
                    on: group.on,
                    active: true,
        ...
        operationFor( line: RuleLine, event: string ): string {
            return this.operation( line, event ) === 'debit'
                ? __( 'Posts as Debit' )
                : __( 'Posts as Credit' );
        },
        isBalanced( group: RuleGroup ): boolean {
            if ( group.lines.length < 2 || group.lines.some( (line) => ! line.amount_source || ( ! line.account_id && ! line.dynamic_account_role ) ) ) {
                return false;
            }
            ...
            if ( group.on === 'order_finalized' ) {
                return this.has( operations, 'debit', 'total' )
            ...
            if ( group.on === 'order_refund' ) {
                return this.has( operations, 'debit', 'net_refund' )
        ...
        balanceSummary( group: RuleGroup ): string {
            ...
            return this.isBalanced( group )
                ? __( 'Balanced  -  {debits} debit action(s), {credits} credit action(s)' ).replace( '{debits}', String( debits ) ).replace( '{credits}', String( credits ) )
