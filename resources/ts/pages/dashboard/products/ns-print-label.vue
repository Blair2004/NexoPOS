            ...
            <div class="shadow-lg bg-white" id="label-printing-paper" :style="{ width: ( form.document_size + 'px' ) || 'auto' }">
                <div class="grid" :class="'grid-cols-' + ( form.max_columns || 1 )">
                    <div class="item border border-black" :style="itemsStyle" v-for="item of itemsToPrint">
                        ...
                        <div :class="layout.centered_content ? 'flex justify-center py-1 text-center' : 'flex justify-between py-1'" v-if="visibility.show_barcode_text && getItemBarcode( item )">
                        ...
                        <div class="flex justify-center flex-col py-1" v-if="getItemBarcode( item )">
                            ...
                            <ul>
                                <li v-for="product of products" :key="product.id" class="border border-box-elevation-edge bg-box-elevation-background p-2 flex justify-between items-center" style="margin-bottom:-1px;">
                                    <p class="flex flex-col">
                                        ...
                                        <span class="text-xs">{{ getItemBarcode( product ) }}</span>
                                    </p>
...
export default defineComponent({
    ...
    mounted() {
        ...
        this.fields         =   validation.createFields([
            {
                ...
                type: 'number',
                label: __( 'Vertical Padding (pixels)' ),
                name: 'veritcal_padding',
            }, {
                type: 'number',
                label: __( 'Horizontal Padding (pixels)' ),
                name: 'horizontal_padding',
            }, {
                type: 'number',
                label: __( 'Barcode Height (pixels)' ),
                name: 'barcode_height',
                value: 30
            }, {
                type: 'number',
                label: __( 'Document Size (pixels)' ),
                name: 'document_size',
                value: ''
            }
        ]);
        ...
        this.visibilityFields   =   [
            {
                type: 'checkbox',
                label: __( 'Show Store Name' ),
                name: 'show_store_name',
                value: true,
            }, {
                type: 'checkbox',
                label: __( 'Show Barcode Text' ),
    ...
    methods: {
        ...
        getItemBarcode( item ) {
            ...
            if ( productBarcode && productBarcode !== '0' && productBarcode !== 0 ) {
