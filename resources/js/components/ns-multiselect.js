    ...
    <div class="bg-white flex flex-col" :class="showPanel ? 'shadow' : ''">
        <div @click="showPanel = !showPanel" :class="showPanel ? 'm-2' : ''" class="select-preview flex justify-between rounded border-2 border-gray-200 p-2 items-center">
            <div class="flex -mx-1 -my-1 flex-wrap">
                <div class="px-1 my-1" v-for="(option,index) of field.options.filter( o => o.selected )">
        ...
        <div class="h-0" v-if="showPanel">
            <div class="bg-white shadow">
                ...
                <div class="h-40 overflow-y-auto">
                    <div @click="addOption( option )" v-for="option of field.options" :class="option.selected ? 'bg-blue-300 text-white' : 'text-gray-600'" class="option p-2 flex justify-between cursor-pointer hover:bg-blue-200 hover:text-white">
                ...
                <div v-if="field.options.length === 0" class="p-2 text-center text-gray-400">Nothing to display</div>
            </div>
        </div>
        </div>
    `,
